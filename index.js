var api = "introduce_api_key_here"; // Put alphavantage api key here
var dps = [];
var company = null;
var symbol = null;
var chart = null;
...
//Getting choosen company from the user and drawing chart and table
...
function setData() {
  const values = ["Facebook Inc(FB)", "Microsoft Corp(MSFT)",
    "Alphabet Inc(GOOG)", "Tesla Inc(TSLA)", "Snap Inc(SNAP)"];
  for (let i = 0; i < values.length; i++) {
    document.getElementsByTagName('option')[i].value = values[i];
  }
}
//Drawing table
function drawTable() {
